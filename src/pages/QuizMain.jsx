...
import { addQuiz } from "../feature/quiz/quizSlice";
...
const QuizMain = () => {
  ...
  const handleQuizLogin = async (e) => {
    e.preventDefault();
    console.log(questionFormData.quizId);
    const data = await fetchQuizById(questionFormData.quizId);
    console.log(data);
    console.log(questionFormData)
    navigate(`/quiz-page/${questionFormData.quizId}?metricId=${questionFormData.metricId}`);
  };

  return (
    <div className="sm:w-[50%]  my-20 mx-5  sm:mx-auto lg:w-[500px] drop-shadow-lg bg-white ">
      <form className="p-12 " onSubmit={handleQuizLogin}>
        <h1 className="backdrop-blur-sm text-4xl pb-8">Login</h1>
        <div className="space-y-5">
          <label htmlFor="quizId" className="block">
            Quiz Id
          </label>
          ...
          <label htmlFor="vId" className="block">
           Metric ID
          </label>
          <div className="relative">
            <input
